...
docker run --rm --name collectdata  -v /etc/localtime:/etc/localtime:ro -v /etc/TZ:/etc/timezone:ro -e TZ=Europe/Berlin -v /volume2/SSD/scripts/dwh-docker:/usr/src/myapp -w /usr/src/myapp  marctv/python-pymysql:latest sh -c "python dwhproxy.py writeweather 'Hannover,de' && python dwhproxy.py writetwitter 'MarcTV' && python dwhproxy.py writeyoutube && python dwhproxy.py writedockerhub 'marctv' 'minecraft-papermc-server' && python dwhproxy.py writedockerhub 'marctv' 'minecraft-bedrock-server' && python dwhproxy.py writedockerhub 'marctv' 'minecraft-overviewer' && python dwhproxy.py writeblog && python dwhproxy.py writemcserver && python dwhproxy.py writemctps '${tps}' && python dwhproxy.py writemccpu '${cpu}' && python dwhproxy.py writemcmem '${mem}'"
